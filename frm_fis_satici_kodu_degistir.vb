...
Public Class frm_fis_satici_kodu_degistir
    ...
    Private Sub frm_fis_depo_kodu_degistir_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        ...
        eskiDepoKodu.Properties.DataSource = sorguC.sorgu_DS(sorgu_query("SELECT sSaticiRumuzu, (sAdi + ' ' + sSoyadi) as sAciklama FROM tbSatici")).Tables(0)
        yeniDepoKodu.Properties.DataSource = sorguC.sorgu_DS(sorgu_query("SELECT sSaticiRumuzu, (sAdi + ' ' + sSoyadi) as sAciklama FROM tbSatici")).Tables(0)
    ...
    Private Sub SimpleButton1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SimpleButton1.Click
        If CheckEdit1.Checked = True Then
            kriter = "(sSaticiRumuzu = '" & Trim(eskiDepoKodu.Text) & "')"
            mesaj = "Eski Satýcý Kodu: " & Trim(eskiDepoKodu.Text) & "," & vbCrLf & "Yeni Satýcý Kodu: " & Trim(yeniDepoKodu.Text)
            ...
            If XtraMessageBox.Show(mesaj, Sorgu_sDil("Dikkat", sDil), MessageBoxButtons.YesNo, MessageBoxIcon.Question) = MsgBoxResult.Yes Then
                ...
                Me.Enabled = False
                sorguC.sorgu_tam("SET DATEFORMAT DMY Update tbStokFisiDetayi Set sSaticiRumuzu = '" & Trim(yeniDepoKodu.Text) & "' Where " & kriter)
                sorguC.sorgu_Destek("Satýcý Hareketleri Kaydýrma", mesaj.Substring(0, mesaj.Length - 27), "SimpleButton1_Click()", "frm_fis_satici_kodu_degistir.vb")
                MessageBox.Show("Satýcý Hareketleri Kaydýrma iþlemi baþarýlý.", "Business Smart")
                PanelControl2.Visible = False
                Me.Enabled = True
                Me.Close()
            End If
        Else
            If XtraMessageBox.Show("Eski Satýcý Kodu: " & Trim(eskiDepoKodu.Text) & "," & vbCrLf & "Yeni Satýcý Kodu: " & Trim(yeniDepoKodu.Text) & vbCrLf & "Ýþlemi Onaylýyormusunuz...?", Sorgu_sDil("Dikkat", sDil), MessageBoxButtons.YesNo, MessageBoxIcon.Question) = MsgBoxResult.Yes Then
                PanelControl2.Visible = True
                PanelControl2.BringToFront()
                Me.Enabled = False
                sorguC.sorgu_tam("Update tbStokFisiDetayi Set sSaticiRumuzu = '" & Trim(yeniDepoKodu.Text) & "' Where sSaticiRumuzu = '" & Trim(eskiDepoKodu.Text) & "'")
                sorguC.sorgu_Destek("Satýcý Hareketleri Kaydýrma", "Eski Satýcý Kodu: " & Trim(eskiDepoKodu.Text) & ", Yeni Satýcý Kodu: " & Trim(yeniDepoKodu.Text) & "", "SimpleButton1_Click()", "frm_fis_satici_kodu_degistir.vb")
                MessageBox.Show("Satýcý Hareketleri Kaydýrma iþlemi baþarýlý.", "Business Smart")
                PanelControl2.Visible = False
                Me.Enabled = True
                Me.Close()
