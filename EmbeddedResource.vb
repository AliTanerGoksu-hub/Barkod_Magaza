...
Friend Class EmbeddedResource
    ...
    '--
    Private Shared Sub GetAssembly(ByVal strResourceName As String)
        If _objMyAssembly Is Nothing Then
            _objMyAssembly = System.Reflection.Assembly.GetExecutingAssembly
        End If
        If _strAssemblyName = "" Then
            For Each s As String In _objMyAssembly.GetManifestResourceNames()
                If s.IndexOf("." & strResourceName) > -1 Then
                    _strAssemblyName = s.Substring(0, s.IndexOf(strResourceName))
                    Exit For
                End If
            Next
        End If
