...
#Region " API Declarations - GetSQLServers   "
    ...
    Protected Declare Function NetServerEnum _
       Lib "netapi32.dll" Alias "NetServerEnum" (
          ...
          ByVal servertype As EServerTypes,
          <MarshalAs(UnmanagedType.LPWStr)> ByVal domain As String,
          ByVal resume_handle As Integer
          ) As Integer
    Protected Declare Function NetApiBufferFree _
       Lib "netapi32.dll" Alias "NetApiBufferFree" (
...
#Region " PROTECTED - Variable declarations  "
    ...
    'Private WithEvents m_ConSQL As OleDb.OleDbConnection
#End Region
#Region " Event declarations                 "
    Public Event ConnectionStateChange(ByVal sender As Object, ByVal e As StateChangeEventArgs)
...
#End Region
    ...
    End Sub
    Public Sub New(ByVal SQLServerName As String,
                   ByVal SQLDatabaseName As String,
                   ByVal SQLUserID As String,
                   ByVal SQLPassword As String, ByVal connection As String,
        ...
        If StartDB Then
            m_DatabaseStarted = StartDatabase(m_ServerName, m_DatabaseName, m_UserID, m_Password, connection)
        Else
            m_DatabaseStarted = False
        End If
    End Sub
    ...
    '  StartDatabase
    '*******************************************
    Public Function StartDatabase(Optional ByVal SQLServerName As String = "",
                                  Optional ByVal SQLDatabaseName As String = "",
                                  Optional ByVal SQLUserID As String = "",
                                  Optional ByVal SQLPassword As String = "", Optional ByVal connection As String = ""
                                  ) As Boolean
        ...
        If bInterrupt Then
            ...
            Exit Function
        End If
        If ((Len(SQLServerName) = 0) Or
           (Len(SQLDatabaseName) = 0) Or
           (Len(SQLUserID) = 0)) Then
            'All the parameters were empty
            If ((Len(m_ServerName) = 0) Or
               (Len(m_DatabaseName) = 0) Or
               (Len(m_UserID) = 0)) Then
        ...
        'sConnectionString = "Data Source=(local);Initial Catalog=NEBIM;Integrated Security=True"
        sConnectionString = connection
    ...
    '  GetSQLServers
    '*******************************************
    'Original C# code by Eugene E. Zhukovsky, 8.21.2002
    'VB.NET port by Charl Pohlmann, 4.08.2003
    Public Overloads Shared Function GetSQLServers(ByVal ServerType As EServerTypes, ByVal Domain As String) As ServerInfo()
        ...
        Const etriesread As Integer = 0
        Dim oEntries() As ServerInfo
        Try
            If NetServerEnum(Nothing, 101, ppSVINFO, -1, etriesread, 0, ServerType, Domain, 0) = 0 Then
                ...
                For i = 0 To etriesread - 1
                    ...
                    oEntries(i) = New ServerInfo(si.sv101_platform_id,
                                                 si.sv101_name,
                                                 si.sv101_version_major,
    ...
    End Function
    Private Sub m_ConSQL_StateChange(ByVal sender As Object, ByVal e As StateChangeEventArgs) Handles m_ConSQL.StateChange
        ...
        If DatabaseName.Length = 0 Then
            ...
            If m_ServerName.Length > 0 Then
                ...
                Try
                    sConnectionString = ""
                    DatabaseExist = False
                    sConnectionString = String.Format("Data Source={0};Initial Catalog=master;User Id={1};Password={2};Connect Timeout={3}", m_ServerName, m_UserID, m_Password, m_TimeOut)
            ...
            Else
                DatabaseExist = False
                Throw New SQLDLNoServerSpecifiedException("No Server specified!",
    ...
    Public Function GetDatabaseLocation(ByVal DatabaseName As String) As String
        ...
        If m_ServerName.Length > 0 Then
            ...
            Try
                ...
                oSQLConnection.ConnectionString = sConnectionString
                oSQLConnection.Open()
                oSQLCommand.CommandText = String.Format("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED SELECT filename FROM sysdatabases WHERE name = '{0}'", DatabaseName)
    ...
    Public Function GetDataAdapter(ByVal SelectSQL As String, ByVal CommandType As CommandType) As System.Data.SqlClient.SqlDataAdapter
        ...
        If m_DatabaseStarted Then
            Dim oDC As New SqlClient.SqlCommand(SelectSQL) With {.CommandType = CommandType, .Connection = m_ConSQL}
            Dim oDA As New SqlClient.SqlDataAdapter(oDC)
            oDC.Dispose()
            Return oDA
        Else
            Throw New SQLDLDatabaseNotStartedException("Database not yet started!",
    ...
    Public Overloads Shared Function GetDatabases(ByVal ServerName As String, ByVal Username As String, ByVal Password As String) As String()
        ...
        If ServerName.Length > 0 Then
            ...
            Dim sConnectionString As String
            Try
                sConnectionString = String.Format("Data Source={0};Initial Catalog=master;User Id={1};Password={2};Connect Timeout=15", ServerName, Username, Password)
                oSQLConnection.ConnectionString = sConnectionString
                oSQLConnection.Open()
                oSQLCommand.CommandText = "sp_databases"
                oSQLCommand.CommandType = CommandType.StoredProcedure
    ...
    Public Function GetTableNames() As String()
        ...
        If Not m_DatabaseStarted Then
            ...
            Throw New SQLDLDatabaseNotStartedException("Database not started.",
...
Public NotInheritable Class ServerInfo
    ...
    Friend Sub New(ByVal p_PlatformID As UInt32,
                   ByVal p_Name As String,
                   ByVal p_Version_major As UInt32,
                   ByVal p_Version_minor As UInt32,
                   ByVal p_Type As UInt32,
