...
// SqlConnectionString: Varsa kullan, yoksa LicenseConnectionString'den otomatik turet
string GetSqlConnectionString()
{
    ...
    var parts = oleDb.Split(';').Select(p => p.Trim()).Where(p => !string.IsNullOrEmpty(p))
        .Select(p => { var kv = p.Split(new[]{'='}, 2); return kv.Length == 2 ? (kv[0].Trim(), kv[1].Trim()) : (p, ""); })
        .ToDictionary(x => x.Item1.ToLower(), x => x.Item2);
    return $"Data Source={parts.GetValueOrDefault("data source", "")};Initial Catalog={parts.GetValueOrDefault("initial catalog", "")};User ID={parts.GetValueOrDefault("user id", "")};Password={parts.GetValueOrDefault("password", "")};TrustServerCertificate=True";
}
...
// ==================== AI ENDPOINTLERI KALDIRILDI ====================
// AI islemleri artik Desktop VB.NET uygulamasinda yerel olarak yapiliyor:
// - AIService.vb -> RiskAciklamaGetir(), GunSonuOzetGetir(), TahsilatPlaniGetir(), SatisOneriGetir(), PerakendeOzetGetir()
// - Her method: Yerel DB sorgusu + OpenAI API cagrisi
// - Desktop API'ye SqlConnectionString bagimliligi kaldirildi
// - Mobil uygulama icin AI endpointleri New_Backend/AIController.cs'de mevcut
// ===================================================================
