Public Class frm_PaketTipiTanim
    ...
    Public Sub Dataload_tbPaketTipleri()
        Try
            ds_tbPaketTipleri.Tables(0).Clear()
        ...
        adapter.SelectCommand = cmd
        cmd.CommandText = sorgu_query("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED SELECT * FROM tbPaketTipleri WHERE sKod <> '' ORDER BY sKod")
        cmd.Connection = conn
        Dim N As Integer = adapter.Fill(ds_tbPaketTipleri, "Table1")
        conn.Close()
        GridControl1.Focus()
        GridControl1.Select()
    End Sub

    Private Sub tbPaketTipleri_kaydet_yeni(ByVal sKod As String, ByVal sAciklama As String)
        Dim cmd As New OleDb.OleDbCommand
        Dim con As New OleDb.OleDbConnection
        cmd.Connection = con
        con.ConnectionString = connection
        ...
        If con.State = ConnectionState.Closed = True Then
            con.Open()
        End If
        cmd.CommandText = sorgu_query("SET DATEFORMAT DMY SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED INSERT INTO tbPaketTipleri (sKod, sAciklama, bAktif) VALUES ('" & sKod & "', '" & sAciklama & "', 1)")
        cmd.ExecuteNonQuery()
        con.Close()
    End Sub

    Private Sub tbPaketTipleri_kaydet_duzelt(ByVal nID As Integer, ByVal sKod As String, ByVal sAciklama As String)
        Dim cmd As New OleDb.OleDbCommand
        Dim con As New OleDb.OleDbConnection
        cmd.Connection = con
        con.ConnectionString = connection
        ...
        If con.State = ConnectionState.Closed = True Then
            con.Open()
        End If
        cmd.CommandText = sorgu_query("SET DATEFORMAT DMY SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED UPDATE tbPaketTipleri SET sKod = '" & sKod & "', sAciklama = '" & sAciklama & "' WHERE nID = " & nID)
        cmd.ExecuteNonQuery()
        con.Close()
    End Sub

    Private Sub tbPaketTipleri_kaydet_sil(ByVal nID As Integer)
        Dim cmd As New OleDb.OleDbCommand
        Dim con As New OleDb.OleDbConnection
        cmd.Connection = con
        con.ConnectionString = connection
        ...
        If con.State = ConnectionState.Closed = True Then
            con.Open()
        End If
        cmd.CommandText = sorgu_query("SET DATEFORMAT DMY SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED DELETE FROM tbPaketTipleri WHERE nID = " & nID)
        cmd.ExecuteNonQuery()
        con.Close()
    End Sub
    ...
    Public Function sorgu_sKod_kontrol(ByVal sKod As String) As Boolean
        ...
        cmd.Connection = conn
        conn.Open()
        cmd.CommandText = sorgu_query("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED SELECT ISNULL(COUNT(sKod), 0) AS nKayit FROM tbPaketTipleri WHERE sKod = '" & sKod & "' AND sKod <> ''")
    ...
    Private Sub kayit_sil()
        If MessageBox.Show(Sorgu_sDil("Seçili Kaydı Silmek İstediğinize Emin misiniz?", sDil), Sorgu_sDil("Dikkat", sDil), MessageBoxButtons.YesNo, MessageBoxIcon.Question) = MsgBoxResult.Yes Then
            ...
            tbPaketTipleri_kaydet_sil(nID)
            Dataload_tbPaketTipleri()
        End If
    End Sub

    Private Sub btn_kaydet_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_kaydet.Click
        If islem = "Ekle" Then
            tbPaketTipleri_kaydet_yeni(txt_sKod.Text, txt_sAciklama.Text)
            Dataload_tbPaketTipleri()
            PanelControl3.Visible = False
        ElseIf islem = "Duzelt" Then
            Dim nID As Integer = GridView1.GetFocusedRowCellValue("nID")
            tbPaketTipleri_kaydet_duzelt(nID, txt_sKod.Text, txt_sAciklama.Text)
            Dataload_tbPaketTipleri()
            PanelControl3.Visible = False
        End If
    End Sub
    ...
    Private Sub raporla_excel()
        ...
        sfd.FileName = "PaketTipleri_" & DateTime.Now.ToString("yyyyMMdd_HHmmss")
    ...
    Private Sub frm_PaketTipiTanim_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dataload_tbPaketTipleri()
        Try
            gorunum_yukle()
        Catch ex As Exception
