...
Public Class frm_tbBirimCinsi
    ...
    Private Function sorgu(ByVal query As String) As DataSet
        ...
        Return DS
    End Function
    Public Sub Dataload_tbBirimCinsi()
        Try
            ds_tbBirimCinsi.Tables(0).Clear()
        Catch ex As Exception
        End Try
        Dim conn As New OleDb.OleDbConnection
        Dim cmd As New OleDb.OleDbCommand
        ...
        adapter.SelectCommand = cmd
        cmd.CommandText = sorgu_query("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED SELECT * From tbBirimCinsi Where sBirimCinsi <>'' ")
        cmd.Connection = conn
        conn.Open()
        Dim N As Integer = adapter.Fill(ds_tbBirimCinsi, "Table1")
        conn.Close()
        GridControl1.Focus()
        GridControl1.Select()
    End Sub
    Private Sub tbBirimCinsi_kaydet_yeni(ByVal sBirimCinsi As String, ByVal sAciklama As String, ByVal nOndalik As Int64)
        ...
        con.Close()
    End Sub
    Private Sub tbBirimCinsi_kaydet_duzelt(ByVal sBirimCinsi As String, ByVal sAciklama As String, ByVal nOndalik As Int64)
        Dim cmd As New OleDb.OleDbCommand
        Dim con As New OleDb.OleDbConnection
        cmd.Connection = con
        con.ConnectionString = connection
        ...
        If con.State = ConnectionState.Closed = True Then
            con.Open()
        End If
        cmd.CommandText = sorgu_query("SET DATEFORMAT DMY SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED UPDATE    tbBirimCinsi SET             sAciklama = N'" & sAciklama & "', nOndalik = " & nOndalik & " Where sBirimCinsi = '" & sBirimCinsi & "'")
        cmd.ExecuteNonQuery()
        con.Close()
    End Sub
    Private Sub tbBirimCinsi_kaydet_sil(ByVal sBirimCinsi As String)
        Dim cmd As New OleDb.OleDbCommand
        Dim con As New OleDb.OleDbConnection
        cmd.Connection = con
        con.ConnectionString = connection
        ...
        If con.State = ConnectionState.Closed = True Then
            con.Open()
        End If
        cmd.CommandText = sorgu_query("SET DATEFORMAT DMY SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED DELETE FROM tbBirimCinsi Where sBirimCinsi  ='" & sBirimCinsi & "'")
        cmd.ExecuteNonQuery()
        con.Close()
    End Sub
    Public Function sorgu_sBirimCinsi_kontrol(ByVal sBirimCinsi As String) As Boolean
        Dim kriter
        Dim pass As Boolean
        kriter = ""
        Dim conn As New OleDb.OleDbConnection
        ...
        cmd.Connection = conn
        conn.Open()
        cmd.CommandText = sorgu_query("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED SELECT     ISNULL(COUNT(sBirimCinsi), 0) AS nKayit FROM         tbBirimCinsi WHERE     (sBirimCinsi = '" & sBirimCinsi & "') and sBirimCinsi <>'' ")
    ...
    Private Sub kayit_ekle()
        ...
        txt_sBirimCinsi.EditValue = ""
        txt_sAciklama.EditValue = ""
        txt_nOndalik.EditValue = 0
        txt_sBirimCinsi.Enabled = True
        islem = "Ekle"
        txt_sBirimCinsi.Focus()
        txt_sBirimCinsi.Select()
    End Sub
    Private Sub kayit_duzelt()
        ...
        txt_sBirimCinsi.EditValue = dr("sBirimCinsi")
        txt_sAciklama.EditValue = dr("sAciklama")
        txt_nOndalik.EditValue = dr("nOndalik")
        txt_sBirimCinsi.Enabled = False
        islem = "Düzelt"
        dr = Nothing
        txt_sAciklama.Focus()
    ...
    Private Sub kayit_sil()
        If XtraMessageBox.Show(Sorgu_sDil("Kaydý Silmek Ýstediðinize Emin misiniz...?", sDil), Sorgu_sDil("Dikkat", sDil), MessageBoxButtons.YesNo, MessageBoxIcon.Question) = MsgBoxResult.Yes Then
            Dim dr As DataRow = GridView1.GetDataRow(GridView1.FocusedRowHandle)
            tbBirimCinsi_kaydet_sil(dr("sBirimCinsi"))
            Dataload_tbBirimCinsi()
        End If
    End Sub
    Private Sub kaydet()
        If islem = "Ekle" Then
            tbBirimCinsi_kaydet_yeni(txt_sBirimCinsi.EditValue, txt_sAciklama.EditValue, txt_nOndalik.EditValue)
            Dataload_tbBirimCinsi()
            GridView1.FocusedRowHandle = GridView1.RowCount - 1
        ElseIf islem = "Düzelt" Then
            tbBirimCinsi_kaydet_duzelt(txt_sBirimCinsi.EditValue, txt_sAciklama.EditValue, txt_nOndalik.EditValue)
            Dataload_tbBirimCinsi()
    ...
    Private Sub btn_kaydet_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_kaydet.Click
        kaydet()
    End Sub
    Private Sub txt_sBirimCinsi_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles txt_sBirimCinsi.Validating
        If islem = "Ekle" Then
            If sorgu_sBirimCinsi_kontrol(Trim(txt_sBirimCinsi.EditValue)) = False Then
                txt_sBirimCinsi.ErrorText = txt_sBirimCinsi.Text & vbTab & "Kaydý Daha Önce Kullanýlmýþ"
                e.Cancel = True
                txt_sBirimCinsi.Focus()
                txt_sBirimCinsi.SelectAll()
            End If
        End If
    End Sub
