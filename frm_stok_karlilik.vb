...
Imports System.IO
Public Class frm_stok_karlilik
...
#Region " Windows Form Designer generated code "
    ...
    <System.Diagnostics.DebuggerStepThrough()> Private Sub InitializeComponent()
        ...
        Dim resources As System.ComponentModel.ComponentResourceManager = New System.ComponentModel.ComponentResourceManager(GetType(frm_stok_karlilik))
    ...
    "ý ve Envanter Rakamlarýný görmektesiniz..."
        ...
        StyleFormatCondition4.Column = Me.colbFiyatDegisti
        ...
        Me.colONBESAY.VisibleIndex = 10
        Me.colONBESAY.Width = 76
        '
        'colORAN4
        '
        ...
        Me.colORAN4.VisibleIndex = 11
        Me.colORAN4.Width = 82
        '
        'colFIYAT5
        '
        ...
        Me.colOran5.VisibleIndex = 13
        '
        'ContextMenu1
        '
        Me.ContextMenu1.MenuItems.AddRange(New System.Windows.Forms.MenuItem() {Me.MenuItem15, Me.MenuItem16, Me.MenuItem1, Me.MenuItem2, Me.MenuItem3, Me.MenuItem12, Me.MenuItem4, Me.MenuItem19, Me.MenuItem5, Me.MenuItem6, Me.MenuItem21, Me.MenuItem20, Me.MenuItem13, Me.MenuItem7, Me.MenuItem8, Me.MenuItem9, Me.MenuItem14, Me.MenuItem25, Me.MenuItem10, Me.MenuItem11, Me.MenuItem18, Me.MenuItem17})
        '
        'MenuItem15
        '
        ...
        'BarButtonItem3
        '
        Me.BarButtonItem3.Caption = "Stok Fiyalarý,Ctrl+F"
...
#End Region
    ...
    Public Function stok() As DataSet
        ...
        conn.ConnectionString = connection
        adapter.SelectCommand = cmd
        cmd.CommandText = sorgu_query("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED SELECT * FROM (SELECT stok.nStokID, stok.sKodu, stok.sAciklama, stok.sBeden, stok.sModel, barkod.sBarkod, (SELECT isnull(SUM(tbStokFisiDetayi.lGirisMiktar1 - tbStokFisiDetayi.lCikisMiktar1) , 0) FROM tbStokFisiDetayi WHERE Stok.nStokID = tbStokFisiDetayi.nStokID) AS Mevcut, (SELECT isnull(lfiyat , 0) FROM tbstokfiyati WHERE nStokId = stok.NstokId AND sFiyatTipi = '" & sFiyat1 & "') AS PESIN, (SELECT isnull(lfiyat , 0) FROM tbstokfiyati WHERE nStokId = stok.NstokId AND sFiyatTipi = '" & sFiyat3 & "') AS ONAY, (SELECT isnull(lfiyat , 0) FROM tbstokfiyati WHERE nStokId = stok.NstokId AND sFiyatTipi = '" & sFiyat4 & "') AS ONBESAY, (SELECT isnull(lfiyat, 0) FROM tbstokfiyati WHERE nStokId = stok.NstokId AND sFiyatTipi = '" & sFiyat5 & "') AS FIYAT5, (SELECT isnull(lfiyat , 0) FROM tbstokfiyati WHERE nStokId = stok.NstokId AND sFiyatTipi = '" & sFiyat2 & "') AS ALTIAY,(SELECT isnull(lfiyat , 0) FROM tbstokfiyati WHERE nStokId = stok.NstokId AND sFiyatTipi = '" & sFiyatM & "') AS MALIYET, tbRenk.sRenkAdi, stok.sRenk,(SELECT COUNT(DISTINCT nStokID) AS nKayit FROM tbStokFiyati WHERE (dteFiyatTespitTarihi = '" & Today & "') AND (sFiyatTipi IN ('" & sFiyat1 & "', '" & sFiyat2 & "', '" & sFiyat3 & "')) and nStokID = Stok.nStokID) bFiyatDegisti,CAST(0 AS MONEY) AS ORANEKMALIYET, CAST(0 AS MONEY) AS ORAN1, CAST(0 AS MONEY) AS ORAN2, CAST(0 AS MONEY) AS ORAN3, CAST(0 AS MONEY) AS ORAN4, CAST(0 AS MONEY) AS ORAN5, CAST(0 AS MONEY) AS SONALISMIKTARI, CAST(0 AS MONEY) AS SONALISFIYATI, CAST(0 AS MONEY) AS SONALISMALIYETI,CAST(0 AS MONEY) AS ONCEKIALISMALIYETI,CAST('01.01.1900' AS DATETIME) AS ONCEKIALISTARIHI,CAST('01.01.1900' AS DATETIME) AS SONALISTARIHI,CAST(0 AS MONEY) AS NETMALIYET,CAST(0 AS MONEY) AS EKMALIYET FROM tbStok stok INNER JOIN tbStokBarkodu barkod ON stok.nStokID = barkod.nStokID and barkod.nKisim = 0 INNER JOIN tbRenk ON stok.sRenk = tbRenk.sRenk " & kriter & ") AS T  " & kriter2 & "  ORDER BY sKodu")
        cmd.Connection = conn
        conn.Open()
